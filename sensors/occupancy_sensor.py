...
Publishes to:  parking/{lot_id}/spot/{spot_id}/occupancy  (MQTT)
               SmartParkingDataQueue                     (SQS, when USE_SQS=1)
"""
...
def run():
    client = mqtt.Client(client_id="occupancy-sensor", protocol=mqtt.MQTTv311)
    ...
    print(f"[Occupancy] Simulating {len(sensors)} spots  →  "
          f"publishing every {OCCUPANCY_INTERVAL}s to MQTT")
